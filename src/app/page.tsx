  ...
  const submitPassport = async () => {
    ...
    try {
      const response = await fetch("https://api.passport.xyz/registry/submit-passport", {
          ...
          body: JSON.stringify({ address, scorer_id: GITCOIN_SCORER_ID }),
      });
      if (response.ok) {
        setTimeout(() => { fetchGitcoinScore([address]); setIsSubmittingPassport(false); }, 2000);
      }
    } catch (e) { setIsSubmittingPassport(false); }
  };
    ...
    <div className="min-h-screen bg-background text-foreground p-6 font-mono overflow-x-hidden relative flex flex-col transition-colors duration-300">
      ...
      <div id="header-anim" className="flex items-center justify-between mb-8 pb-4 border-b border-border">
          <div className="flex items-center gap-4">
              ...
              >
                  <MdContentPasteSearch className="text-primary-foreground w-7 h-7" />
              </motion.div>
              <div className="flex flex-col justify-center">
                  ...
                  <p className="text-[8px] text-muted-foreground mt-1 font-bold tracking-widest uppercase">Reputation Score</p>
              </div>
          </div>
      ...
      <div id="profile-card" className="bg-card/50 backdrop-blur-md p-6 rounded-2xl border border-primary/20 mb-6 shadow-xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 mb-6 relative z-10">
             {farcasterUser?.pfpUrl && (
              <img src={farcasterUser.pfpUrl} alt="PFP" className="w-14 h-14 rounded-full border-2 border-border shadow-md"/>
            )}
            <div>
              <div className="flex items-center gap-2">
                ...
                <div id="verification-status">
                    {isIdentityVerified && isSocialVerified ? (
                      <span className="bg-primary/20 px-2 py-0.5 rounded border border-primary/50 flex items-center gap-1 animate-pulse">
                          ...
                          <span className="text-[9px] font-bold text-primary tracking-wider">VERIFIED</span>
                      </span>
                    ) : (isIdentityVerified || isSocialVerified) ? (
                      <span className="bg-green-500/10 px-2 py-0.5 rounded border border-green-500/50 flex items-center gap-1">
                          <ShieldCheck className="w-3 h-3 text-green-500" />
                          <span className="text-[9px] font-bold text-green-500">PARTIAL</span>
                      </span>
                    ) : (
                      <span className="bg-destructive/10 px-2 py-0.5 rounded border border-destructive/50 flex items-center gap-1">
                          <AlertTriangle className="w-3 h-3 text-destructive" />
                          <span className="text-[9px] font-bold text-destructive">UNVERIFIED</span>
                      </span>
                    )}
                </div>
              </div>
